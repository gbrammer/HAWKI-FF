...
\caption{\textit{Left:} Difference in the derived rest-frame $U-V$ colors as a function of photometric redshift for galaxies in the MACS-0416 parallel field, before and after including $K_s$ photometry along with the \textit{HST} and \textit{Spitzer} measurements.  The scatter in the derived $U-V$ colors at $z>2.5$ is large ($\sim$0.2 mag) where the Balmer/4000~\AA\ break is in the gap between \textit{HST} $H_{160}$ and the \textit{Spitzer} IRAC 3.6~\mum\ bands, which is well then constrained by including $K_s$.  This large scatter translates directly into large scatter on other quantities of interest, such as the stellar mass to light ratio and the age of the stellar populations.  The center and right panels show a dramatic example of a galaxy with a strong break between the \textit{HST} and IRAC bands.  The addition of the $K_s$ band pinpoints the location of the break and shrinks the photometric redshift uncertainties by a factor of two and suggesting $z_\mathrm{phot} > 3$ (right panel). \label{fig:delta_UV}}  
\end{figure*}
