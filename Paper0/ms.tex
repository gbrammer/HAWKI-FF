...
%                                                                %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{deluxetable*}{ccccrccc} 
\tabletypesize{\footnotesize}
\tablewidth{0pt} 
\tablecaption{Field summary \label{tab:summary}} 
\tablehead{ 
\colhead{Field} & \colhead{R.A.} & \colhead{Dec.} & \colhead{Instrument} & \colhead{Epoch} & \colhead{$t$} & \colhead{Depth$^\mathrm{a}$} & \colhead{FWHM}}
\startdata 
...
Abell S1063  & $22:49:01.1$ & $-44:32:13$ & \textit{VLT}/HAWKI     & 2015 Jul 8--Sep 22 & 25.5 h & 25.9 & $0\farcs38$ \\ 
Abell 370    & $02:40:03.3$ & $-01:36:23$ & \textit{VLT}/HAWKI     & 2015 Jul 26--2016 Jan 28 & 25.5 h & 25.9 & $0\farcs38$ \\ 
MACS-0717    & $07:17:34.0$ & $+37:44:49$ & \textit{Keck}/MOSFIRE  & 2015 Jan 26--27, 2016 Jan 21--22  & 0/0 h\footnotemark[2] & 25.0/25.0 & $0\farcs43$/$0\farcs00$ \\ 
MACS-1149    & $11:49:36.3$ & $+22:23:58$ & \textit{Keck}/MOSFIRE  & 2015 Feb 24, 2016 Jan 21--22      & \xxx\footnotemark[2] & \xxx & \xxx \\
''           & ''           & ''          & \textit{VLT}/HAWKI\footnotemark[3]     & 2013 Mar 21--2014 Jun 9      & \xxx & \xxx & \xxx
\enddata 
%\vspace{-0.8cm} 
\footnotetext[1]{Depth is defined as the 5$\sigma$ limiting magnitude for point sources, measured in $D=0\farcs6$ apertures (\S\ref{s:depth}).} 
\footnotetext[2]{The MOSFIRE observations of MACS-0717 and MACS-1149 require two separate pointings of the instrument to cover the cluster/parallel \textit{HST} fields, which have the different characteristics as indicated, respectively.}
...
The product of Eq.~\ref{eq:aper_var} and the inverse of the curves of growth shown in Fig.~\ref{fig:cog} (i.e., aperture corrections) defines an optimal photometric aperture size where this product is minimized \citep{whitaker:nmbs}.  For our $K_s$-band mosaics, this optimal aperture has $D\sim0\farcs6$, just larger than the FWHM of point-source profiles.  The $5\,\sigma$ limiting magnitudes (AB) within $0\farcs6$ diameter apertures are \xxx\ 25.8, 25.9, and 25.0 in the Abell 2744, MACS-0416, and MACS-0717 fields, respectively.  The HAWK-I images presented here are among the deepest $K_s$-band images ever obtained and will provide a crucial complement to the deep \textit{HST} and \textit{Spitzer} imaging of the Frontier Fields.  

We conclude here with a brief comment on the ``HUGS'' ultra-deep HAWK-I images of the CANDELS survey fields.  \cite{hugs} report a final depth of 26.5 AB for point sources within $D=0\farcs4$ apertures for their deepest field, ``GOODS-D1'', which has an exposure time of 31.5 hours similar to the deep HAWK-I fields summarized in Table~\ref{tab:summary}.  \cite{hugs} assume ideal noise properties that likely suffer some residual correlations between adjacent pixels such as that indicated by the red curves in Fig.~\ref{fig:corr_noise}, where we found that the drizzled rms was some 40\% higher than that measured in the presence of the pixel correlations for $0\farcs1$ pixel mosaics.  This effect ($\sim$0.4 mag) along with the relative exposure times ($\sim$0.1 mag) can account for much of the difference between the reported depths of the deepest HUGS pointings and the \textit{Hubble} Frontier Fields HAWK-I images described here.  The comparison must be made because any additional half magnitude increase depth is exponentially more expensive to obtain!
...
\caption{Image cutouts of the MACS 0416 parallel field.  The \textit{RGB} and monochrome panels come from the sources as indicated; the cutouts are all $10\arcsec$ \xxx\ [check] on a side.
\label{fig:cutout}}  
\end{figure}
