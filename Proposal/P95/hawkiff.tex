...
Discretionary (DD) time, the Hubble Space Telescope (\textit{HST}) is embarking in a continuing
...
\MakeCaption{Fig.~1. \textit{left panels:} Layout of the existing P92 (PI: Brammer) and proposed P95 HAWKI pointings on the four HST Frontier Fields targets visible from the VLT.  The $7.5^\prime\times7.5^\prime$ HAWKI field of view is perfectly suited to provide \textit{simultaneous} ultra-deep $K_s$-band imaging of both the cluster and parallel HST fields.  \textit{right panels:} Cutout of the MACS 0416 field, indicated by the small box within the full field at left.  The existing and proposed HAWKI $K_s$-band imaging provide a critical scientific complement to the ultra-deep HST survey, with exquisite image quality ($0\farcs4$) and sensitivity reaching $K=26.3$ AB (\textit{total}, 5$\sigma$, point source).}

\MakePicture{P95_f2.pdf}{angle=0,width=17cm}
\MakeCaption{Fig.~2. \textit{a)} Cumulative ($N(>z)$) number counts of sources with $K < 26.5$ in the area of overlap with deep WFC3/IR imaging (8 arcmin$^2$ per target). Only a few bright $z\sim8$ candidates are seen in each field (see panel c.4), so \textit{all} of the FF clusters must be observed to maximize the high-$z$ sample.  \textit{b)} Without $K$-band photometry that samples rest-frame optical wavelengths at $z>2.5$, the uncertainty in stellar mass estimates is extremely large ($\sigma > 1$ dex). \textit{Panels c.1---4}) Example SED fits from the P92 clusters.  \textit{c.1} is a remarkable evolved galaxy at $z\sim2.8$ with a prominent bulge and an extended grand-design spiral disk. \textit{c.2} and \textit{c.3} show significant evolved components only only apparent in the $K$-band photometry.  \textit{c.4} is a robust $z\sim8.6$ LBG candidate with an unambiguous $K$-band detection ($K\sim26$) that constrains its rest-frame UV spectral slope.  }
