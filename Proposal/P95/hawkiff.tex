...
\Title{Ultra-Deep $K_s$-band imaging of the HST Frontier Fields Year 3 Clusters}  
...
\Abstract{The HST Frontier Fields (HFF) program is a multi-cycle program 
...
the reddest HST filter (1.6\,$\mu\mathrm{m}$~$\sim$~$H$) and the 
...
\noindent {\bf The HST Frontier Fields:} Through a large investment (560 
orbits) of Director's Discretionary (DD) time, the Hubble Space Telescope 
(HST) is embarking in a continuing multi-cycle program to further investigate 
the most distant universe ($z$$>$$5$). The HST Frontier Fields (HFF) program 
has already imaged two deep fields centered on strong lensing galaxy clusters 
in parallel with two deep blank fields over the past cycle.  Four more clusters
and deep fields are planned for the next two cycles.  The primary science 
...
with IRAC (3.6$\mu$m and 4.5$\mu$m bands) with roughly 1000 hours of 
...
dataset will be unique for its combination of surveyed area and depth in 
...
fill the large gap in the wavelength coverage between the HST $H_{160}$ and 
IRAC 3.6\,$\mu\mathrm{m}$ filters of the ongoing HST Frontier Fields survey. 
...
unprecedented combination of depth and area.  The addition of the $K$-band 
...
redshifts ([5], see also Fig.~2c) and increase the wavelength lever arm for 
...
\vspace{0.1cm} \hspace{0.5cm} We will use the combined HST+HAWK-I dataset to 
...
depths [9,4,11,12]. The deep HST photometry alone will be insufficient to 
...
Fig.~2b); the $K$-band is required to select galaxies at rest-frame optical 
wavelengths free from confusion challenges of the redder IRAC bands. The 
...
\vspace{0.1cm} \hspace{0.5cm} We are committed to providing a valuable 
resource to the community to complement the large investment of \textit{VLT} 
time: we waive the proprietary period on the HAWK-I observations. Our team has 
extensive experience processing deep $K$-band observations (including HAWK-I, 
[5]) and we are committed to a public release of reduced, registered mosaics 
within one year of completing the integration on each of the two survey 
fields, as demonstrated for the awarded P92 program ({http://github.com/gbrammer/HAWKI-FF}).
...
by the 4 HST ACS+WFC3 pointings covered here with HAWK-I. Scaling the mass 
...
$\sim$$0\farcs45$ in the $K$-band is necessary to match, as well as possible, 
the high-resolution HST imaging ($0\farcs18$ in $H_{160}$) of the crowded HFF 
...
41$\times$2=82 hours for the full integrations on the final two southern HST 
...
HST Frontier Fields, we require a large telescope aperture and an infrared 
...
large aperture telescope currently mounts a NIR imager wide enough to cover 
both the cluster and parallel fields at once. The proposed ultra-deep HAWK-I 
...
\textbf{092.A-0472}, ``Ultra-Deep $K_s$-band imaging of the HST Frontier Fields'', 82 h, HAWK-I.  This program observed the first two FF clusters; see Figs. 1 \& 2.  The images have been reduced and mosaics will be released to the community Fall 2014, accompanied by a survey description paper.
